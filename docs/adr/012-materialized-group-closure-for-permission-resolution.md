...
WHERE relationship_name = 'project_member'
  ...
  AND from_party_id IN (
    $userPartyID,
    SELECT ancestor_party_id FROM party_group_closure WHERE member_party_id = $userPartyID
  )
```
...
set from `party_group_closure` in `r.Context()` on the first resolution per request.
...
**Rebuild scope:** The closure rebuild is scoped to the affected group — only rows where
`ancestor_party_id = changedGroupID` are deleted and re-inserted. A full-table rebuild is
never required.
...
- Write operations that change group membership pay the cost of closure rebuild (synchronous,
  same transaction). For large groups this adds latency to membership changes — acceptable
  since membership changes are infrequent compared to reads
- `party_group_closure` is a derived table; it must never be edited directly
- Circular group membership (A member of B, B member of A) would cause infinite recursion
  during rebuild — the store layer must detect and reject cycles before committing
...
- Adding a new containment relationship kind requires adding it to `ContainmentRelationships`
  to trigger rebuilds; forgetting to do so causes stale closure data (silent bug)
