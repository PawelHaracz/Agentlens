...
│                                                                   │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌───────────────────┐ │
│  │ REST API │  │  Web UI  │  │Discovery │  │  Health Checker   │ │
│  │  (Chi)   │  │ (React)  │  │ Manager  │  │   (Plugin)        │ │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └─────────┬─────────┘ │
│       │             │             │                   │           │
│  ┌────▼─────────────▼─────────────▼───────────────────▼────────┐  │
│  │              Auth Middleware (JWT / RequirePermission)       │  │
│  └────────────────────────────┬─────────────────────────────────┘  │
│                               │                                   │
│                    ┌──────────▼──────────┐                        │
│                    │   Kernel (Core)     │                        │
...
│      │                        │                        │          │
│  ┌───▼───────┐     ┌──────────▼────────┐   ┌──────────▼───────┐  │
│  │  Database │     │  Parser Plugins   │   │  Source Plugins  │  │
│  │  (GORM)   │     │  (A2A, MCP)       │   │  (Static, K8s)   │  │
│  │SQLite/PG  │     └───────────────────┘   └──────────────────┘  │
│  └───────────┘                                                    │
└───────────────────────────────────────────────────────────────────┘
```
...
Login Request
     │
     ▼
UserStore.GetByUsername()
     │
     ▼
CheckPassword() [bcrypt cost 12]
     │
     ├── Fail → IncrementFailedAttempts() → lock after 5 fails (15 min)
     │
     └── Pass → ResetFailedAttempts()
                     │
                     ▼
               JWTService.GenerateToken(user, role)
                     │
                     ▼
               Set httpOnly Secure cookie + return token
```
...
```
Browser                  API                  UserStore / JWTService
   │                      │                          │
   │  POST /auth/login     │                          │
   │─────────────────────►│                          │
   │                      │  GetByUsername()          │
...
```
HTTP Request → Chi Router → RequireAuth() → RequirePermission() → API Handler → Store → DB
                                │
                                └── Unauthenticated → 401
                                └── Insufficient perm → 403
```
